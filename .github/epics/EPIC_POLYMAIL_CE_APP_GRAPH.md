...
- [ ] **P1.1** — Implement `polymail_app_graph.fl` with 13 module definitions and `make_polymail_module` helper
- [ ] **P1.2** — Wire `EDGE_REQUIRES` dependency edges in `polymail_app_graph_register`
- [ ] **P1.3** — Implement `polymail_register_bridge_edges` (4 bridges: PolyKit metering, PolyKit RBAC, PolyKit sanitize, PolyCalendar)
- [ ] **P1.4** — Implement `polymail_register_governance_edges` (13 `EDGE_GOVERNANCE_OBSERVE`)
...
- [ ] **P3.2** — Implement `configure_polymail_noise_filter` (suppress newsletters, auto-replies; signal delivery failures, encryption downgrades)
...
- [ ] **P3.6** — Implement `polymail_register_ce` orchestrator and `full_ce_pipeline_setup` golden test

### Phase 4: Integration & Validation

- [ ] **P4.1** — Verify all 13 modules resolve after registration
- [ ] **P4.2** — Verify bridge edges connect to live PolyKit and PolyCalendar module graphs
...
1. `polymail_app_graph_register` produces a `CsrStorage` with exactly 13 nodes
2. `polymail_register_bridge_edges` adds 4 bridge nodes with `EDGE_BRIDGE_TO` edges
3. `polymail_register_governance_edges` adds 13 `EDGE_GOVERNANCE_OBSERVE` edges
...
| File | Description |
|------|-------------|
| `circuits/fl/polymail_app_graph.fl` | 13-module app graph, edges, bridges, governance, tests |
| `circuits/fl/polymail_meaning.fl` | 3 CE domains, noise filter, 2 SME panels, orchestrator, tests |
